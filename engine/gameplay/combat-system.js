...
export class CombatSystem {
  startCombat() {
    const engine = window.gameEngine; // Zugriff auf die globale Engine
    const player = engine.stateManager.getState().player;

    // Wähle ein zufälliges Monster für den Testkampf
    const monsterList = Object.values(COMMON_MONSTERS);
    const randomMonsterData =
      monsterList[Math.floor(Math.random() * monsterList.length)];

    // Erstelle eine frische Kopie des Monsters für den Kampf
    const monster = JSON.parse(JSON.stringify(randomMonsterData));

    engine.stateManager.updateState("currentView", "combat");
    engine.stateManager.updateState("combat", {
      player: player,
      monster: monster,
    });

    engine.stateManager.addLog(`Ein wildes ${monster.name} erscheint!`);
  }
  ...
  performAction(state, action) {
    if (!state.combat) {
        console.warn("Kampfaktion wurde aufgerufen, aber kein Kampf ist aktiv.");
        return;
    }
    ...
    switch (action.type) {
      case "attack": {
        ...
        log.push(
          `${player.name} greift an und verursacht ${damageDealt} Schaden.`
        );
        break;
      }
      case "spell": {
        ...
        if (spell && player.mp >= spell.costMp) {
          ...
          if (spell.effect.type === "damage") {
            const damageDealt =
              spell.effect.amount + (player.stats.intelligence || 5);
            monster.hp -= damageDealt;
            log.push(
              `${player.name} wirkt ${spell.name} für ${damageDealt} Schaden!`
            );
          } else if (spell.effect.type === "heal") {
            player.hp = Math.min(player.maxHp, player.hp + spell.effect.amount);
            log.push(
              `${player.name} wirkt ${spell.name} und heilt ${spell.effect.amount} HP.`
            );
          }
        } else {
          log.push(
            `Nicht genug Mana für ${spell ? spell.name : "diesen Zauber"}!`
          );
        }
        break;
      }
      case "item": {
        ...
        if (itemIndex > -1) {
          ...
          if (item.effect.type === "heal") {
            ...
            log.push(
              `${player.name} benutzt ${item.name} und heilt ${item.effect.amount} HP.`
            );
          }
      ...
      if (lootTable) {
        ...
        lootTable.items.forEach((lootItem) => {
          if (Math.random() < lootItem.chance) {
            const newItem = {
              ...lootItem.item,
              lootId: `loot_${Date.now()}_${Math.random()}`,
            };
        ...
        if (lootTable.gold) {
          goldGained =
            Math.floor(
              Math.random() * (lootTable.gold.max - lootTable.gold.min + 1)
            ) + lootTable.gold.min;
        }
      }
    ...
    log.push(
      `${monster.name} greift zurück und verursacht ${damageTaken} Schaden.`
    );
