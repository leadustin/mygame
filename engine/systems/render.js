...
const ALL_ITEMS = { ...WEAPONS, ...ARMOR, ...POTIONS, ...CLOAKS, ...BELTS, ...JEWELRY, ...HANDS, ...FEET, ...ARTIFACTS, ...SHIELDS, ...MATERIALS, ...SPELLS };
...
export class RenderSystem {
    ...
    render(state) {
        ...
        switch (state.currentView) {
            case 'title_screen': this.renderTitleScreenView(); break;
            case 'character_creation': this.renderCharacterCreationView(state); break;
            case 'map': this.renderMapView(state); break;
            case 'combat': this.renderCombatView(state); break;
            case 'game_over': this.renderGameOverView(); break;
            case 'post_combat_loot': this.renderPostCombatLootView(state); break;
            default: this.mainView.innerHTML = `<h2>Unbekannte Ansicht: ${state.currentView}</h2>`;
        }
    ...
    attachDragDropListeners(container) {
        ...
        container.addEventListener('dragend', (e) => { if (e.target.tagName === 'IMG') e.target.style.opacity = '1'; });
        container.addEventListener('dragover', (e) => {
            const targetSlot = e.target.closest('.drop-target');
            if (targetSlot) { e.preventDefault(); targetSlot.classList.add('drag-over'); }
        });
        ...
        container.addEventListener('drop', (e) => {
            ...
            if (targetSlot) {
                ...
                const targetData = {
                    type: targetSlot.classList.contains('equipment-slot') ? 'equipment' : 'inventory',
                    slot: targetSlot.dataset.slot,
                    index: targetSlot.dataset.index
                };
                eventBus.publish('inventory:itemMoved', { source: sourceData, target: targetData });
            }
        });
    }
    ...
    renderHud() {
        ...
        this.hudContainer.addEventListener('click', (e) => {
            ...
            switch(action) {
                ...
                case 'startTestCombat': eventBus.publish('game:startCombat'); break;
            }
        });
    ...
    renderCharacterCreationView(state) {
        ...
        const renderStepIndicator = () => {
            ...
            el.innerHTML = steps.map((name, i) => `<div class="step ${currentStep === i + 1 ? 'active' : ''}">${i + 1}. ${name}</div>`).join('');
        };
        const renderNavButtons = () => {
            const el = this.mainView.querySelector('#cc-nav-buttons');
            const back = `<button id="cc-back-btn" ${currentStep === 1 ? 'disabled' : ''}>Zurück</button>`;
            const next = `<button id="cc-next-btn">${currentStep === steps.length ? 'Abenteuer beginnen' : 'Weiter'}</button>`;
            el.innerHTML = back + next;
            el.querySelector('#cc-back-btn').addEventListener('click', () => { currentStep--; renderWizard(); });
            el.querySelector('#cc-next-btn').addEventListener('click', handleNextClick);
        };
        const renderContentPanel = () => {
            const el = this.mainView.querySelector('#cc-content-panel');
            switch(currentStep) {
                case 1: renderSelectionStep(el, RACES, 'raceId'); break;
                case 2: renderSelectionStep(el, CLASSES, 'classId'); break;
                case 3: renderPersonalizationStep(el); break;
                case 4: renderFinalizationStep(el); break;
            }
        };
        const renderSelectionStep = (container, data, key) => {
            ...
            Object.entries(data).forEach(([id, value]) => {
                ...
                btn.className = `selection-button ${creationData[key] === id ? 'active' : ''}`;
                btn.dataset.id = id;
            ...
            renderDetailsPanel(container.querySelector('#cc-details-panel'), data[creationData[key]]);
        };
        const renderDetailsPanel = (container, details) => {
            let statsHtml = '';
            if (details.baseStats) {
                for (const [stat, val] of Object.entries(details.baseStats)) statsHtml += `<div class="stat-line"><span>${stat}</span><span>${val}</span></div>`;
            } else if (details.statModifiers) {
                for (const [stat, val] of Object.entries(details.statModifiers)) statsHtml += `<div class="stat-line"><span>${stat}</span><span>${val > 0 ? '+' : ''}${val}</span></div>`;
            }
            let equipHtml = '';
    ...
    renderCombatView(state) {
        ...
        const usablePotions = player.inventory.filter(i => i && i.type === 'potion');
        const knownSpells = player.spellbook.map(spellId => Object.values(SPELLS).find(s => s.id === spellId)).filter(s => s);

        this.mainView.innerHTML = `
            <div id="combat-screen" style="padding: 20px; text-align: center;">
                <div class="monster-area" style="margin-bottom: 40px;"><h2>${monster.name}</h2><p>HP: ${monster.hp} / ${monster.maxHp}</p></div>
                <div class="player-area" style="margin-bottom: 40px;"><h3>${player.name}</h3><p>HP: ${player.hp} / ${player.maxHp}</p><p>MP: ${player.mp} / ${player.maxMp}</p></div>
                <div class="actions" style="margin-bottom: 20px;"><button id="attack-btn">Angriff</button><button id="spell-btn" ${knownSpells.length === 0 ? 'disabled' : ''}>Zauber</button><button id="item-btn" ${usablePotions.length === 0 ? 'disabled' : ''}>Item</button></div>
                <div id="combat-submenu-container" style="min-height: 50px;"></div>
            </div>`;

        const submenuContainer = this.mainView.querySelector('#combat-submenu-container');
        ...
        document.getElementById('spell-btn').addEventListener('click', () => {
            submenuContainer.innerHTML = knownSpells.map(spell => `<button class="spell-choice-btn" data-spell-id="${spell.id}" ${player.mp < spell.costMp ? 'disabled' : ''}>${spell.name} (${spell.costMp} MP)</button>`).join('');
        });
        document.getElementById('item-btn').addEventListener('click', () => {
            submenuContainer.innerHTML = usablePotions.map(item => `<button class="item-choice-btn" data-item-id="${item.id}">${item.name}</button>`).join('');
        });
        this.mainView.addEventListener('click', (e) => {
            if (e.target.matches('.spell-choice-btn')) eventBus.publish('combat:action', { type: 'spell', spellId: e.target.dataset.spellId });
            if (e.target.matches('.item-choice-btn')) eventBus.publish('combat:action', { type: 'item', itemId: e.target.dataset.itemId });
        });
    }
    ...
    renderPostCombatLootView(state) {
        ...
        this.mainView.innerHTML = `<div id="loot-screen-container"><div id="loot-panel"><h2>SIEG!</h2><p>Du erhältst ${postCombatState.xpGained} Erfahrungspunkte.</p><div id="loot-gold">Gefunden: ${postCombatState.goldGained} Gold</div><h4>Beute</h4><div id="loot-list">${postCombatState.loot.length === 0 ? '<p>Keine Beute gefunden.</p>' : ''}</div><div id="loot-actions"><button id="take-selected-btn">Nehmen</button><button id="dismantle-btn">Zerlegen</button><button id="take-all-btn">Alles nehmen</button><button id="close-loot-btn">Schließen</button></div></div></div>`;
        ...
        lootListEl.addEventListener('click', e => { e.target.closest('.loot-item')?.classList.toggle('selected'); });
        this.mainView.querySelector('#take-selected-btn').addEventListener('click', () => {
            const ids = Array.from(lootListEl.querySelectorAll('.loot-item.selected')).map(el => el.dataset.lootId);
            if (ids.length > 0) eventBus.publish('loot:take_selected', ids);
        });
        this.mainView.querySelector('#dismantle-btn').addEventListener('click', () => {
            const ids = Array.from(lootListEl.querySelectorAll('.loot-item.selected')).map(el => el.dataset.lootId);
            if (ids.length > 0) eventBus.publish('loot:dismantle', ids);
        });
        this.mainView.querySelector('#take-all-btn').addEventListener('click', () => eventBus.publish('loot:take_all'));
        this.mainView.querySelector('#close-loot-btn').addEventListener('click', () => eventBus.publish('loot:close'));
    }
    ...
    renderActiveWindows(state, container) {
        ...
        state.activeWindows.forEach(name => {
            ...
            else if (name === 'inventory') this.renderInventoryWindow(el, state.player);
    ...
    renderCharacterWindow(windowEl, player) {
        ...
        const updateStatsDisplay = () => {
            ...
            const baseStats = CharacterSystem.getBaseStats(player);
            let primaryStatsHtml = '';
            const primaryStats = ['strength', 'dexterity', 'intelligence', 'vitality'];
            primaryStats.forEach(stat => {
                const value = tempStats[stat] || 0;
                const decreaseBtn = tempStats[stat] > originalStats[stat] ? `<button class="decrease-stat-btn stat-mod-btn" data-stat="${stat}">-</button>` : '';
                const increaseBtn = tempUnspentPoints > 0 ? `<button class="increase-stat-btn stat-mod-btn" data-stat="${stat}">+</button>` : '';
            ...
            secondaryStats.forEach(stat => {
                if (tempStats[stat] !== undefined) secondaryStatsHtml += `<div class="stat-line"><span>${stat.charAt(0).toUpperCase() + stat.slice(1)}</span><span>${tempStats[stat]}</span></div>`;
            });
            const pointsDisplay = player.unspentStatPoints > 0 ? `<div class="stat-points-display">Verfügbare Punkte: ${tempUnspentPoints}</div>` : '';
            const confirmButton = tempUnspentPoints < player.unspentStatPoints ? `<button class="confirm-stats-btn">Punkte bestätigen</button>` : '';
            statsPanel.innerHTML = `
                ...
                <hr>
                <div class="stat-line"><span><img src="assets/images/icons/gold_coins.webp" alt="Gold">Gold</span><span>${player.gold}</span></div>
                ${pointsDisplay}
                <hr>
