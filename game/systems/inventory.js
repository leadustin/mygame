...
export class InventorySystem {
  ...
  static handleItemMove(player, source, target) {
    ...
    if (
      (source.source === "inventory" || source.source === "equipment") &&
      (target.type === "merchant" || target.type === "shop")
    ) {
      let itemToSell;
      let sellData = {
        item: null,
    ...
    if (
      (source.source === "merchant" || source.source === "buyback") &&
    ...
    if (
      ...
      !target.slot.startsWith(sourceItem.type)
    ) {
      return { player, log: [] };
    }
    if (
      ...
      !source.slot.startsWith(targetItem.type)
    ) {
      return { player, log: [] };
    }
    ...
    if (target.type === "inventory") {
      if (target.index === undefined || target.index === null) {
        const emptyIndex = player.inventory.findIndex(
  ...
  takeSelectedLoot(lootIds) {
    const engine = window.gameEngine;
    const state = engine.stateManager.getState();
    if (!state.postCombatState) return;

    const player = { ...state.player };
    const postCombat = { ...state.postCombatState };
    const itemsToTake = [];

    // Finde die Items basierend auf den IDs und entferne sie aus der Beuteliste
    postCombat.loot = postCombat.loot.filter((item) => {
      if (lootIds.includes(item.lootId)) {
        itemsToTake.push(item);
        return false; // Entferne das Item aus der Liste
      }
      return true; // Behalte das Item in der Liste
    });

    // Füge die Items dem Spielerinventar hinzu
    itemsToTake.forEach((item) => {
      delete item.lootId; // Die temporäre lootId wird nicht mehr gebraucht
      player.inventory.push(item);
    });

    engine.stateManager.updateState("player", player);
    engine.stateManager.updateState("postCombatState", postCombat);
    engine.stateManager.addLog(
      `${itemsToTake.length} Gegenstand/Gegenstände genommen.`
    );
  }
  ...
  takeAllLoot() {
    const engine = window.gameEngine;
    const state = engine.stateManager.getState();
    if (!state.postCombatState || state.postCombatState.loot.length === 0)
      return;
    ...
    postCombat.loot.forEach((item) => {
      delete item.lootId;
      player.inventory.push(item);
    });

    engine.stateManager.addLog(
      `Alle ${postCombat.loot.length} Gegenstände genommen.`
    );

    postCombat.loot = []; // Leere die Beuteliste

    engine.stateManager.updateState("player", player);
    engine.stateManager.updateState("postCombatState", postCombat);
  }

  dismantleLoot(lootIds) {
    // Logik für das Zerlegen von Items (kannst du später implementieren)
    const engine = window.gameEngine;
    engine.stateManager.addLog("Zerlegen ist noch nicht implementiert.");
  }
  ...
  closeLoot() {
    ...
    if (levelUpResult.leveledUp) {
      levelUpResult.log.forEach((msg) => engine.stateManager.addLog(msg));
    }

    // Setze den Zustand zurück und gehe zur Weltkarte
  ...
  static recalculateStats(player) {
    ...
    for (const stat in raceData.statModifiers) {
      finalBaseStats[stat] =
        (finalBaseStats[stat] || 0) + raceData.statModifiers[stat];
    }
